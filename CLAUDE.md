...
- **Design system:** colors and fonts are Tailwind tokens in `tailwind.config.js` (`paper`, `ink`, `muted`, `line`, `cobalt`, `zellige`, `brass`; `font-sans` = Bricolage Grotesque for headings and UI, `font-serif` = Source Serif 4 for body text, loaded in `public/index.html`). Shared classes (`.section-title`, `.btn-primary`, `.btn-secondary`, `.text-link`) are in `src/index.css`. The eight-point star motif (nav mark, hero portrait, chat button) comes from `src/components/common/ZelligeStar.jsx`. The hero star's draw-in is the only animation that plays without the visitor doing anything; keep it that way.
