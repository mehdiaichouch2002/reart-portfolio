...
Personal portfolio for Mehdi Aichouch: a single-page Create React App site (React 18, Tailwind CSS 3) with no router and no backend.
...
- **i18n (EN/FR):** `src/context/LanguageContext.jsx` provides `t("dotted.key")`, which looks up `src/i18n/translations.js` and returns the key itself when nothing matches. The chosen language is saved in `localStorage["lang"]`. `t()` can also return arrays or objects. For example, the Experience timeline is `t("timeline")`; each title is written as `"Organisation — role"` and `About.jsx` splits it on the em dash. Any user-facing text change has to be made in both the `en` and `fr` trees.
- **Projects:** `src/data/projects.js` holds the metadata: image import, link, `hosted`, `category` (`commercial` | `opensource` | `personal`), `tech`, and `descriptionKey`. The description text is at `portfolio.projects.<descriptionKey>` in both languages. `commercial` projects are shown twice: as the "live stores" links in the hero and as large rows at the top of `Portfolio.jsx`. Everything else goes in the list below, with Magento projects sorted first. Project images go in `src/assets/projects/`.
- **Skills** are a hardcoded `SKILL_GROUPS` object in `Skills.jsx`; only the group names are translated. Contact and social URLs live in `src/data/contactLinks.js`.
- **Design system:** dark "holographic chrome" theme. Colors and fonts are Tailwind tokens in `tailwind.config.js` (`canvas`, `surface`, `fg`, `muted`, `line`, `accent`, `live`, `danger`; `font-display` = Syne for headings and the wordmark, `font-sans` = Instrument Sans for everything else, loaded in `public/index.html`). Shared classes (`.section-title`, `.btn-primary`, `.btn-secondary`, `.text-link`, `.chrome-text`, `.chrome-frame`) are in `src/index.css`. The iridescent `.chrome-text` gradient is reserved for the hero headline, and its one-time sweep is the only animation that plays without the visitor doing anything; keep both that way. There is no logo mark, only the text wordmark.
...
- The `@anthropic-ai/sdk`, `@google/generative-ai`, `axios`, `react-slick`, `slick-carousel` and `lucide-react` dependencies are currently unused in `src/`.
...
- `src/setupTests.js` mocks `matchMedia`, `IntersectionObserver` (for framer-motion), and `scrollIntoView`. It also clears `localStorage` after each test so the saved language doesn't carry over into the next test.
