...
- `src/setupTests.js` mocks `matchMedia` (react-slick), `IntersectionObserver` (framer-motion), `scrollIntoView`, and clears `localStorage` after each test (language choice persists there and leaks between tests otherwise).
...
Key checks:
- `document.documentElement.scrollWidth <= viewport` at 320/390/768/1024/1440. Note `html,body { overflow-x: clip }` masks overflow from scrollWidth — also scan `getBoundingClientRect()` of all elements for rects past the viewport edges. Expected false positives: mobile sidebar parked at `-translate-x-full`, framer-motion `whileInView` elements still at their initial x-offset, slick-track carousel clones.
- Hero typewriter: the longest role is French "Développeur Full-Stack" — set `localStorage.setItem("lang","fr")`, reload, `waitForFunction` for the full string, then measure the h2 role line. It must fit the text column (container is `max-w-screen-lg`, so the side-by-side column is ~592px).
